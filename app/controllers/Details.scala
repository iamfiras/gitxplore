...
object Details extends Controller {
  ...
  def getTimeline(commitsFuture: Future[Seq[Commit]]) = Action.async {
    for {
      commits <- commitsFuture
    } yield Ok(views.html.details.timeline(commits))
  }

  def getContributions(commitsFuture: Future[Seq[Commit]]) = Action.async {
    val label = "%s (%2.2f%%, %d commits)"
    for {
      commits <- commitsFuture
    } yield {
      val grouped = commits groupBy { commit => commit.committer }
      val contributions = grouped map { commitList =>
        (label.format(commitList._1, (commitList._2.length.toDouble / commits.length.toDouble) * 100, commitList._2.length), commitList._2)
      }
      Ok(views.html.details.contributions(contributions.toList.sortBy(- _._2.length)))
    }
  }
